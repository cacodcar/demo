...
class F:
    ...
    Args:
        one (int | float | list[int | float] | P | V | F, optional): First element.
        two (int | float | list[int | float] | P | V | F, optional): Second element. Defaults to 0.
    ...
    Attributes:
        ...
        array (list[P | V]): List of elements in the function
        vars (list[V]): List of variables in the function
        struct (list[P | V | str]): Structure of the function
        ...
        elmo (dict[int, list[P | V | str]]): Elements in the function with relation. Also a sesame street character
    ...
    def __init__(
        ...
        if not self.isnnvar and mis < 1:
            # two is longer
            one_ = [x for x in self.one._ for _ in range(-mis)]
            two_ = self.two._

        elif not self.isnnvar and mis > 1:
        ...
        self.array = sum(
            [i.array if isinstance(i, F) else [i] for i in [self.one, self.two]],
            [],
        )

        self.vars = [i for i in self.array if not isinstance(i, P)]

        _rels = []
        if isinstance(self.one, F):
            _rels += self.one.rels

        _rels += [self.rel]

        if isinstance(self.two, F):
            _rels += self.two.rels

        _struct = []
        for n, e in enumerate(self.array):
            if n > 0:
                _struct.append(_rels[n - 1])
            _struct.append(e)

        if isinstance(_struct[0], P) and _struct[0].isneg():
            _struct = ['-', -_struct[0]] + _struct[1:]
        else:
            _struct = ['+'] + _struct

        self.struct = _struct
        self.rels = _rels
        self.elms = _struct[1::2]
        self._rels = _struct[::2]

        self._elms = [
            sum(
                [
                    i.elms if isinstance(i, Func) else [i],
                    j.elms if isinstance(j, Func) else [j],
                ],
                [],
            )
        ...
        for n, _ in enumerate(one_):
            self.elmo[n] = sum([[i, j] for i, j in zip(self._rels, self._elms[n])], [])
        ...
        # self.idx = {}
        self.args = {'mul': self.mul, 'add': self.add, 'sub': self.sub, 'div': self.div}
