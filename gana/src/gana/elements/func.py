...
class Func:
    ...
    def __init__(
        ...
        pos: int = None,
        struct: list[str | int | float | Var] = None,
    ):
        ...
        self.n = None
        self.struct = [
            -i if isinstance(i, (int, float)) and i < 0 else i for i in struct
        ]
        self.elms = [e for e in self.struct if not isinstance(e, str)]
        # self.elms = [-i if isinstance(i, (int, float)) and i < 0 else i for i in elms]
        self.vars = [i for i in self.struct if not isinstance(i, (str, int, float))]

        self.name = ''.join([str(i) for i in self.struct])
    ...
    def B(self, zero: bool = False) -> int | float | None:
        ...
        if isinstance(self.struct[-1], float):
            if self.struct[-2] == '+':
    ...
    def A(self) -> list[float]:
        ...
        x = self.struct
        if isinstance(self.struct[-1], float) and self.struct[-2] in ['+', '-']:
