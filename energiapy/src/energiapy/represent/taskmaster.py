...
from .._core import _Dunders
# from ...components.analytical.player import Player
from ..components import (Cash, Emission, Resource, _BalanceAttrs, _BoundAttrs,
                          _BoundBoundAttrs, _ExactAttrs)
...
class Chanakya(_BoundAttrs, _BoundBoundAttrs, _ExactAttrs, _BalanceAttrs, _Dunders):
    ...
    def __post_init__(self):
        ...
        # self.detriment = Bound(sibling=self.emit)
        # Resource | Land
        ...
        # self.buy_abate = Calculate(parent=self.buy, ilk=self.sequester)
        # self.sell_emit = Calculate(parent=self.sell, ilk=self.emit)
        # self.sell_abate = Calculate(parent=self.sell, ilk=self.sequester)
        # Lose
        # self.lose_emit = Calculate(parent=self.lose, ilk=self.emit)
        # self.recover_abate = Calculate(parent=self.recover, ilk=self.sequester)
        # Use/Dispose
        # self.use_emit = Calculate(parent=self.use, ilk=self.emit)
        # self.use_abate = Calculate(parent=self.use, ilk=self.sequester)
        # self.dispose_emit = Calculate(parent=self.dispose, ilk=self.emit)
        # self.dispose_abate = Calculate(parent=self.dispose, ilk=self.sequester)
        # Setup/Dismantle
        # self.setup_emit = Calculate(parent=self.setup, ilk=self.emit)
        # self.setup_abate = Calculate(parent=self.setup, ilk=self.sequester)
        # self.dismantle_emit = Calculate(parent=self.dismantle, ilk=self.emit)
        # self.dismantle_abate = Calculate(parent=self.dismantle, ilk=self.sequester)
        # Operate
        # self.operate_emit = Calculate(parent=self.operate, ilk=self.emit)
        # self.operate_abate = Calculate(parent=self.operate, ilk=self.sequester)
        # ---------------Use/Dispose----------------
        # Setup
