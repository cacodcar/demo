...
from ...energiapy.src.energiapy.components.temporal.period import Period
...
def test_period(m):
    ...
    assert not m.h.motion
    assert m.d.periods == 24
    assert m.d.motion == m.h
    assert m.y.periods == 8760
    assert m.y.motion == m.h
    assert m.w.motion == m.h
